...
-- This migration captures the current state of the EM-V1 database
-- Use this to migrate to a new Supabase project or local database
...
DO $$
BEGIN
    -- Add syncStatus and lastSynced to companies if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'syncStatus') THEN
        ALTER TABLE companies ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'lastSynced') THEN
        ALTER TABLE companies ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'isLocal') THEN
        ALTER TABLE companies ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to events if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'events' AND column_name = 'syncStatus') THEN
        ALTER TABLE events ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'events' AND column_name = 'lastSynced') THEN
        ALTER TABLE events ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'events' AND column_name = 'isLocal') THEN
        ALTER TABLE events ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to attendees if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendees' AND column_name = 'syncStatus') THEN
        ALTER TABLE attendees ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendees' AND column_name = 'lastSynced') THEN
        ALTER TABLE attendees ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendees' AND column_name = 'isLocal') THEN
        ALTER TABLE attendees ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to company_users if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'company_users' AND column_name = 'syncStatus') THEN
        ALTER TABLE company_users ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'company_users' AND column_name = 'lastSynced') THEN
        ALTER TABLE company_users ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'company_users' AND column_name = 'isLocal') THEN
        ALTER TABLE company_users ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to lucky_draw_winners if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lucky_draw_winners' AND column_name = 'syncStatus') THEN
        ALTER TABLE lucky_draw_winners ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lucky_draw_winners' AND column_name = 'lastSynced') THEN
        ALTER TABLE lucky_draw_winners ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lucky_draw_winners' AND column_name = 'isLocal') THEN
        ALTER TABLE lucky_draw_winners ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to voting_sessions if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_sessions' AND column_name = 'syncStatus') THEN
        ALTER TABLE voting_sessions ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_sessions' AND column_name = 'lastSynced') THEN
        ALTER TABLE voting_sessions ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_sessions' AND column_name = 'isLocal') THEN
        ALTER TABLE voting_sessions ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to voting_options if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_options' AND column_name = 'syncStatus') THEN
        ALTER TABLE voting_options ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_options' AND column_name = 'lastSynced') THEN
        ALTER TABLE voting_options ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_options' AND column_name = 'isLocal') THEN
        ALTER TABLE voting_options ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to voting_votes if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_votes' AND column_name = 'syncStatus') THEN
        ALTER TABLE voting_votes ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_votes' AND column_name = 'lastSynced') THEN
        ALTER TABLE voting_votes ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_votes' AND column_name = 'isLocal') THEN
        ALTER TABLE voting_votes ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add created_at to voting_votes if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'voting_votes' AND column_name = 'created_at') THEN
        ALTER TABLE voting_votes ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    -- Add syncStatus and lastSynced to quiz_sessions if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_sessions' AND column_name = 'syncStatus') THEN
        ALTER TABLE quiz_sessions ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_sessions' AND column_name = 'lastSynced') THEN
        ALTER TABLE quiz_sessions ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_sessions' AND column_name = 'isLocal') THEN
        ALTER TABLE quiz_sessions ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to quiz_questions if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_questions' AND column_name = 'syncStatus') THEN
        ALTER TABLE quiz_questions ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_questions' AND column_name = 'lastSynced') THEN
        ALTER TABLE quiz_questions ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_questions' AND column_name = 'isLocal') THEN
        ALTER TABLE quiz_questions ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to quiz_participants if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_participants' AND column_name = 'syncStatus') THEN
        ALTER TABLE quiz_participants ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_participants' AND column_name = 'lastSynced') THEN
        ALTER TABLE quiz_participants ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_participants' AND column_name = 'isLocal') THEN
        ALTER TABLE quiz_participants ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to quiz_winners if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_winners' AND column_name = 'syncStatus') THEN
        ALTER TABLE quiz_winners ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_winners' AND column_name = 'lastSynced') THEN
        ALTER TABLE quiz_winners ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quiz_winners' AND column_name = 'isLocal') THEN
        ALTER TABLE quiz_winners ADD COLUMN isLocal BOOLEAN DEFAULT false;
    END IF;
    
    -- Add syncStatus and lastSynced to gallery_uploads if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gallery_uploads' AND column_name = 'syncStatus') THEN
        ALTER TABLE gallery_uploads ADD COLUMN syncStatus TEXT DEFAULT 'synced';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gallery_uploads' AND column_name = 'lastSynced') THEN
        ALTER TABLE gallery_uploads ADD COLUMN lastSynced TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gallery_uploads' AND column_name = 'isLocal') THEN
