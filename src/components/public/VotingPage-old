...
export default function VotingPage() {
  ...
  const fetchPhotosWithVotes = async () => {
    ...
    try {
      ...
      // Fetch vote counts and check if user voted for each photo
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Find attendee by identification number
      const { data, error } = await supabase
        .from('attendees')
      ...
      // Verify attendee belongs to the event of this voting session
  ...
  const handleVote = async (photoId: string) => {
    ...
    try {
      ...
      // Check if user already voted for any photo in this session
